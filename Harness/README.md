...
- `subject.stands(at:sizedWithin:)` — at a frame the scene worked out, the size included, which `isWhereItWas` says nothing about: a window still filling the screen is as much at its old origin as one put back. `Screen.swift` works out the frames the window actions take, mirroring `Core/OffscreenParkingDesktop.swift` and `Core/Model/Half.swift` rather than importing them, so a change only one of them makes is a run that fails. The size is allowed more room than the origin because an application answers a size it can take where it keeps the position it is handed, Terminal to the nearest whole row.
