...
/// The frame each maximized window is to go back to. Tabs of one window share the record:
/// they stand at one frame, and every tab holds the frame so closing the one the maximize
/// went through leaves the rest with it.
final class MaximizedWindows {
    ...
    func record(_ outcomes: [FrameOutcome]) {
        for outcome in outcomes {
            switch outcome {
            case let .maximized(windowId, from): keep(from, of: windowId)
