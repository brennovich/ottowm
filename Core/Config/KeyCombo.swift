...
struct KeyCombo: Hashable {
    ...
    static func parse(_ text: String) -> Result<KeyCombo, ConfigError.Reason> {
        ...
        guard let keyCode = Self.keyCode(named: key) else {
            return .failure(.unknownKey(key))
        }

        var modifiers: [ModifierKey: ModifierSide] = [:]
        for name in components {
            // One name can stand for several modifiers, so "hyper" adds all four at once.
    ...
    // Exact match: a modifier the combo does not name must be absent, which is what
    // keeps cmd-1 and ctrl-1 from triggering a binding on alt-1.
    ...
    // The escape hatch for a key this table does not name. Bounded by CGKeyCode, so a code
    // macOS could never deliver is rejected instead of binding to nothing.
    private static func keyCode(named name: String) -> Int64? {
        guard name.hasPrefix(keyCodePrefix) else { return keyCodesByName[name] }

        return CGKeyCode(name.dropFirst(keyCodePrefix.count)).map(Int64.init)
    }
