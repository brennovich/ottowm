...
final class OriginalFramesTests: XCTestCase {
    ...
    private lazy var originalFrames = OriginalFrames(tabs: { [weak self] in self?.tabs[$0] ?? [$0] })
    ...
    func testASecondFillKeepsTheFrameFromBeforeTheFirst() {
        originalFrames.record([.filled(100, from: original)])
        originalFrames.record([.filled(100, from: filled)])
    ...
    func testRelocateFitsEveryFrameIntoTheNewVisibleFrame() {
        ...
        originalFrames.relocate(with: fit)
    ...
    func testRestoringOneTabEndsTheFillForTheWholeWindow() {
        ...
        originalFrames.record([.active(200)])
    ...
    func testTheFrameOutlivesTheTabTheFillWentThrough() {
        tabs = [100: [100, 200], 200: [100, 200]]
        originalFrames.record([.filled(200, from: original)])

        tabs = [100: [100], 200: [200]]
        originalFrames.forget(200)
    ...
    func testATabThatJoinsAFilledWindowTakesItsFrame() {
        ...
        tabs = [100: [100, 200], 200: [100, 200]]
        originalFrames.shareFrame(with: 200)

        tabs = [200: [200]]
        originalFrames.forget(100)

        XCTAssertEqual(originalFrames.originalFrame(of: 200), original)
    }

    func testAWindowThatIsGoneKeepsItsFrameUntilItIsForgotten() {
        originalFrames.record([.filled(100, from: original)])
        originalFrames.record([.gone(100)])
