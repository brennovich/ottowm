...
final class EngineMaximizeTests: EngineTestCase {
    ...
    func testTheFirstMaximizeHasNothingToRestore() {
        let win = focus(100)
    ...
    func testTheNextMaximizeHandsBackTheFrameTheWindowCameFrom() {
        focus(100)
        engine.handle(.toggleMaximize)
        desktop.clearCalls()
    ...
    func testUnmanagingAWindowDropsTheFrameToRestore() {
        ...
        XCTAssertNil(maximizedWindows.restoringFrame(of: win.id))
    }
}
